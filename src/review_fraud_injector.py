...
class FraudInjectionConfig:
    ...
    user_pick_subset_size: int = 1024
    product_pick_subset_size: int = 2048
...
class ReviewKGUserFraudInjector:
    def __init__(self, kg, cfg: FraudInjectionConfig):
        ...
        self.criminal_pool_list: List[int] = []
        ...
        self.product_degree: Dict[int, int] = {}
        self.product_mean_rating_cache: Dict[int, float] = {}
        self.global_products_low_degree: List[int] = []
        self.global_products_high_degree: List[int] = []
        self.group_products_low_degree: Dict[int, List[int]] = {}

        self._next_review_index = len(self.reviews) + 1

        self._build_indices()
        self._build_pick_caches()
    ...
    def _build_pick_caches(self) -> None:
        self.product_degree = {pid: len(self.product_reviews.get(pid, ())) for pid in self.products}
        self.product_mean_rating_cache = {
            pid: (sum(vals) / len(vals)) if vals else 4.0
            for pid, vals in self.product_ratings.items()
        }
        for pid in self.products:
            self.product_mean_rating_cache.setdefault(pid, 4.0)

        self.global_products_low_degree = sorted(
            self.products,
            key=lambda pid: (self.product_degree.get(pid, 0), pid)
        )
        self.global_products_high_degree = list(reversed(self.global_products_low_degree))

        self.group_products_low_degree = {}
        for gid, plist in self.group_to_products.items():
            self.group_products_low_degree[gid] = sorted(
                plist,
                key=lambda pid: (self.product_degree.get(pid, 0), pid)
            )
    ...
    def _select_criminal_pool(self) -> None:
        ...
        self.criminal_pool_list = list(self.criminal_pool)

        node_props = self.kg.node_props
        for uid in self.criminal_pool_list:
            node_props[uid]["fraud"] = True
    ...
    def _product_mean_rating(self, pid: int) -> float:
        return self.product_mean_rating_cache.get(pid, 4.0)

    def _sample_without_replacement_weighted_subset(
        self,
        population: List[int],
        subset_size: int,
        weight_fn,
        ...
        while len(chosen) < k and len(used) < len(available):
            if len(available) <= subset_size:
                candidates = [x for x in available if x not in used]
            else:
                candidates = []
                seen = set()
                target = min(subset_size, len(available) - len(used))
                while len(candidates) < target:
                    x = available[self.rng.randrange(len(available))]
                    if x in used or x in seen:
                        continue
                    seen.add(x)
                    candidates.append(x)
            ...
            weights = [max(1e-9, float(weight_fn(x))) for x in candidates]
            x = self.rng.choices(candidates, weights=weights, k=1)[0]
            chosen.append(x)
            used.add(x)
    ...
    def _weighted_pick_from_pool(self, k: int) -> List[int]:
        return self._sample_without_replacement_weighted_subset(
            population=self.criminal_pool_list,
            subset_size=max(32, self.cfg.user_pick_subset_size),
            weight_fn=lambda uid: 1.0 + self.cfg.reuse_bias * self.user_fraud_counts[uid],
            k=k,
        )
    ...
    def _candidate_products(
        ...
        if seed_pid is not None and prefer_same_group:
            ...
            if gid is not None:
                cands = self.group_products_low_degree.get(gid, [])
                if cands:
                    return cands if prefer_low_degree else list(reversed(cands))
        return self.global_products_low_degree if prefer_low_degree else self.global_products_high_degree
    ...
    def _pick_products(
        ...
        ordered_candidates = self._candidate_products(
            seed_pid=seed_pid,
        ...
        front_window = min(
            ...
            max(128, min(self.cfg.product_pick_subset_size, max(256, k * 64)))
        )
        front = [pid for pid in ordered_candidates[:front_window] if pid not in avoid_products]
        ...
        step = front_window
        idx = front_window
        while len(chosen) < k and idx < len(ordered_candidates):
            end = min(len(ordered_candidates), idx + step)
            block = [
                pid for pid in ordered_candidates[idx:end]
                if pid not in avoid_products and pid not in chosen_set
            ]
            self.rng.shuffle(block)
            need = k - len(chosen)
            chosen.extend(block[:need])
            chosen_set.update(block[:need])
            idx = end
    ...
    def _register_new_review(self, uid: int, pid: int, rating: float, rid: int) -> None:
        ...
        self.product_ratings[pid].append(float(rating))
        self.user_ratings[uid].append(float(rating))

        self.product_degree[pid] = self.product_degree.get(pid, 0) + 1
        vals = self.product_ratings[pid]
        self.product_mean_rating_cache[pid] = sum(vals) / len(vals)
    ...
    def _add_review(self, uid: int, pid: int, rating: float, count_as_pattern_activity: bool = True) -> bool:
        ...
        rid = self._next_review_index
        self._next_review_index += 1

        review_props = {
            "helpful_vote": 0,
            "images_count": 0,
            "rating": float(self._clip_star(rating)),
            "review_id": f"review_{rid}_{self.rng.randint(1000, 9999)}",
            "text": "",
            "title": "",
            "verified_purchase": bool(self.rng.random() < 0.50),
        }

        new_rid = self.kg.add_node("Review", review_props)
        ok1 = self.kg.add_edge("WROTE", uid, new_rid, {}, no_duplicates=True)
        ok2 = self.kg.add_edge("ABOUT", new_rid, pid, {}, no_duplicates=True)

        if not (ok1 and ok2):
            return False

        self._register_new_review(uid, pid, float(review_props["rating"]), new_rid)
    ...
    def _inject_dense_bipartite(self, remaining: int) -> int:
        ...
        for uid in users:
            reviewed = self.user_products.get(uid, set())
            for pid in products:
